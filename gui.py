...
class ChirpTab(CLTab):
    def __init__(self, project):
        ...
        self.start_freq = CLParameter('Start Freq', self.project['start_freq'], 'Hz')
        self.chirp_params.addWidget(self.start_freq)
        def updateStartFreq():
            new_value = self.start_freq.text_box.text() # get text entered to text box
            if new_value.isnumeric() and float(new_value)>0 and float(new_value)<=(self.project['sample_rate']/2): # make sure new value is a number greater than 0 and less than or equal to Nyquist
                self.start_freq.last_value = new_value # if new value is valid, use the new value
                self.project['start_freq'] = float(new_value) # apply the new value to the project
        ...
        self.stop_freq = CLParameter('Stop Freq', self.project['stop_freq'], 'Hz')
        self.chirp_params.addWidget(self.stop_freq)
        
        self.chirp_length = CLParameter('Chirp Length', self.project['chirp_length'], 'Sec')
        ...
        self.input_file_box = CLParameter('Input File', self.project['input']['file'], '')
        self.input_params.addWidget(self.input_file_box)
        
        self.input_channel = CLParameter('Channel', 1, '')
        ...
        self.sample_rate = CLParameter('Sample Rate', self.project['sample_rate'], 'Hz')
        self.analysis_params.addWidget(self.sample_rate)
        
        self.pre_sweep = CLParameter('Pre Sweep', self.project['pre_sweep'], 'Sec')
        self.analysis_params.addWidget(self.pre_sweep)
        
        self.post_sweep = CLParameter('Post Sweep', self.project['post_sweep'], 'Sec')
    ...
    def updateStimulus(self):
        ...
        self.project['stimulus'] = np.concatenate((
            np.zeros(round(self.project['pre_sweep']*self.project['sample_rate'])),
            logchirp(self.project['start_freq'], self.project['stop_freq'], self.project['chirp_length'], self.project['sample_rate']),
            np.zeros(round(self.project['post_sweep']*self.project['sample_rate']))))
        # update chirp tab graph
        self.graph.axes.cla()
        self.graph.axes.plot(np.arange(len(self.project['stimulus']))/self.project['sample_rate'],self.project['stimulus'])
        self.graph.draw()
        # update measurements
